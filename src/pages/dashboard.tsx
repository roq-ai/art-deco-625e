...
import { getColorPalettes } from 'apiSdk/color-palettes';
...
import { getVersions } from 'apiSdk/versions';
import { VersionInterface } from 'interfaces/version';

import { MotifListPage as ShowCaseList } from 'pages/motifs';

function DashboardPage() {
  const { session } = useSession();

  const colorPaletteStat = useSWR<PaginatedInterface<ColorPaletteInterface>>(
    () => `/color-palettes`,
    () => getColorPalettes({ limit: 1, offset: 0 }),
  );
  ...
  const versionStat = useSWR<PaginatedInterface<VersionInterface>>(
    () => `/versions`,
    () => getVersions({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Color Palettes',
      result: colorPaletteStat?.data?.totalCount,
      isLoading: colorPaletteStat.isLoading,
    ...
    {
      title: 'Versions',
      result: versionStat?.data?.totalCount,
      isLoading: versionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`motifs`} passHref legacyBehavior>
                  <Button
