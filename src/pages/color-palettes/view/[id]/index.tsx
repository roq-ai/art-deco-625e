...
import { getColorPaletteById } from 'apiSdk/color-palettes';
import { ColorPaletteInterface } from 'interfaces/color-palette';

function ColorPaletteViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('color_palette', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/color-palettes/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('gallery', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Gallery"
                    text={
                      <Link as={NextLink} href={`/galleries/view/${data?.gallery?.id}`}>
                        {data?.gallery?.name}
                      </Link>
                    }
                  />
