...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'design.create': {
    roles: ['gallery-owner'],
    key: 'artist-submitted-design',
    tenantPath: ['gallery', 'design'],
    userPath: [],
  },
  'design.update': { roles: ['artist'], key: 'design-approved', tenantPath: ['gallery', 'design'], userPath: [] },
  'motif.update': { roles: ['artist'], key: 'motif-updated', tenantPath: ['gallery', 'motif'], userPath: [] },
};

const ownerRoles: string[] = ['gallery-owner'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['artist'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.gallery.findFirst({
