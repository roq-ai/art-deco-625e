...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['gallery-owner', 'artist'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<GalleryInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/galleries` : null),
    () => getGalleries({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Galleries',
      path: isTenantUser ? `/galleries/view/${data?.[0]?.id}` : '/galleries',
      entity: 'gallery',
    ...
    { name: 'Designs', path: '/designs', entity: 'design', service: AccessServiceEnum.PROJECT, icon: FiLayout },
    { name: 'Motifs', path: '/motifs', entity: 'motif', service: AccessServiceEnum.PROJECT, icon: FiAperture },
    { name: 'Versions', path: '/versions', entity: 'version', service: AccessServiceEnum.PROJECT, icon: FiClock },
